...
Only feature exceeding some given threshold are then used for traning.
...
todo polish this, add sources and wrap it up


\subsubsection{CHI-square~---~$\chi^2$}

CHI-square measures the lack of independence between two variables (CITA).

We consider two variables and for each combination of their vales,
we compute the expected number of instances if these variables were independent.
We will do so, but calculation the distribution of the variables individually.
Then~$\chi^2$~is the following sum for each value combination.

$$\left(O - E\right)^2 \over E),$$

where E is the expected value, and O observed.
...
\subsection{\todoA{Spell check}}

\subsection{\todoA{Review length}}
