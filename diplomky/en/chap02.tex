...
can be \jh{transcribed TO rewritten} with Bayes rule as:
...
\jh{Neni uplne vysvetleny, spis neco jako: Because we are not interested in the exact value of the expression following `arg max` but only it the value maximizing it, we can ignore the denominator:}
...
The \jh{delete: exact} values of probabilities are calculated as follows.
Prior probabilities \jh{do zavorky pridat kus te formule, ktera to je} are the distribution of classes.
The exact values are computed by counts.
For computing posterios \jh{do zavorky pridat kus te formule, ktera to je}, Na\"{i}ve Bayes assumption is used.
...
However, since we are only interested in the label with \jh{the} highest probability,
...
Thanks to the assumption, the probability of an instance represented by \jh{certain} feature values is the product
...
\jh{rest? snad vsechno: 
The probability is the ratio of the number of instances $c_i$ with feature $f_k$~and and all instances of $c_i$. (Ale jak pisu ve slacku, trochu nerozumim oznaceni $f_k$}
...
Let us first explain \jh{a TO the} baseline algorithm on top of which fastText builds.
...
In short, it maps every word onto a \jh{real} vector. Every document is represented as \jh{an TO the} average vector of all words' vectors.

However, fastText goes even further.
The process is precisely described in \citet{Bojanowski2017enriching}\jh{. TO ,}
...
The algorithm represents a \jh{words TO word} as \jh{an TO the} average of vectors of all its \jh{character} n-grams.
...
For even higher performance, \jh{word boundaries are chonsidered characters as well. Technically,} every word is surrounded by \texttt{<} and \texttt{>}.
The word \textit{ where} thus becomes \texttt{<where>}.
The vector of \texttt{where} \jh{becomes TO is} the average of \jh{the} vectors
$w_{<wh}$, 
$w_{whe}$, 
$w_{her}$,
$w_{ere}$ and
